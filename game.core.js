...
var game_core = function(game_instance) {
  ...
  if (this.server) {
    ...
    this.ghosts.server_pos_self.pos = {
      x: 20,
      y: 20
    };
...
game_core.prototype.client_handle_mouse = function(mousePos) {
  this.cursorPos = mousePos
    //console.log(this.cursorPos);
};

game_core.prototype.client_handle_mouseDown = function() {
  var dir = this.v_sub(this.cursorPos, this.players.self.pos);

  dir = this.v_mul_scalar(dir, 1 / Math.sqrt((dir.x * dir.x + dir.y * dir.y)));

  this.synchedObjects.push(new bullet(this.game,
    this.players.self.pos, dir))
  console.log(this.cursorPos)
};
...
game_core.prototype.v_sub = function(a, b) {
  return {
    x: (a.x - b.x).fixed(),
    y: (a.y - b.y).fixed()
  };
...
game_core.prototype.v_mul_scalar = function(a, b) {
  return {
    x: (a.x * b).fixed(),
    y: (a.y * b).fixed()
  };
...
var bullet = function(game_instance, pos, vel) {
  this.game = game_instance;
  this.pos = pos;
  this.vel = vel;
}
...
bullet.prototype.draw = function() {
  ...
  //Draw a rectangle for us
  game.ctx.fillRect(this.pos.x - 2, this.pos.y - 2, 10, 10)
...
game_core.prototype.server_update_physics = function() {
  ...
  for (var i = 0; i < this.synchedObjects.length; i++) {
    this.synchedObjects[i].update()
  }
...
game_core.prototype.server_update = function() {
  ...
  var bulletsPos
  if (this.synchedObjects) {
    bulletsPos = [this.synchedObjects[0]];
  } else {
    bulletsPos = [];
  }
...
game_core.prototype.client_process_net_updates_bullets = function ()
{
  if (!this.server_updates.length) return;
  var count = this.server_updates.length - 1;
  var latest_server_data = this.server_updates[this.server_updates.length - 1];
  if (latest_server_data.b)
  {
    this.synchedObjects[0].pos=latest_server_data.b[0];
  }
...
game_core.prototype.client_onserverupdate_recieved = function(data) {
  ...
  if (this.naive_approach) {
    ...
    this.client_process_net_updates_bullets();
